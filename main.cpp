...
int main() {
    ...
    for (int i = 0; i < width; i++)
        for (int j = 0; j < height; j++) {
            ...
            std::cin >> c;
            pMap[width * j + i] =
                (count(passable.begin(), passable.end(), c) > 0);
    ...
    for (auto PathFinder :
        ...
        for (int t = 0; t < TIMES; t++) {
            ...
            PathFinder.first(
                u.first, u.second, v.first, v.second, pMap, width, height, NULL,
                0
            );
            // printf("(%4d, %4d) -> (%4d, %4d) :: %8d (%.2f)\n", u.first,
            // u.second, v.first, v.second, 	     dist, (dist == -1) ? 0.0 :
            // 100.0 * dist/ExploredNodes);
            tot += ExploredNodes;
        }
        double ticks = clock() - begin_time;
        printf(
            "%20s -- avg time: %-3.3fms\tavg nodes: %8lld\n", PathFinder.second,
            double(ticks / CLOCKS_PER_SEC) * 1000 / TIMES, tot / TIMES
        );
    }
